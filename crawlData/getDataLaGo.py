...
def create_table(sheet_name, arr_name):
    # 创建工作簿
    JobBook = xlwt.Workbook(encoding='utf-8', style_compression=0)
    # 添加一个sheet
    sheet_name = JobBook.add_sheet(sheet_name, cell_overwrite_ok=True)
    # 定义表头
    i = 0
    for title in arr_name:
        sheet_name.write(0, i, title)
        i += 1
    return JobBook
...
if __name__ == '__main__':
    ...
    my_big_tree = header_browser(True,urlPool, Url, User_Agent, Accept, Accept_Charset, Accept_Encoding, Host)
    print(my_big_tree)
    ...
    # 定义表头
    is_English = False
    nameArrC = ['类1','类2','类3','职位名称', '薪资', '工作地', '经验', '学历', '职位类别', '公司名称', '职位诱惑', '职位描述', '详细地址','链接']
    nameArrE = ['class_one','class_two','class_three','Job_name', 'Job_salary', 'Job_area', 'Job_experience', 'Job_education', 'Job_type',
                'Company_name', 'Job_welfare', 'Job_task', 'detail_adder','link_addr']
    nameArr = []
    if is_English:
        nameArr = nameArrE
    else:
    ...
    # 第一级分类
    typeOne=[]
    class1 = my_big_tree.xpath('//div[@id="sidebar"]/div[@class="menu_main job_hopping"]/h2')
    print(class1)
    # test = my_big_tree.xpath('//div[@class="menu_main job_hopping"]/h2')[0]
    # print(removeSpace(test.text))
    # wr = open('url.txt', 'w')
    if len(class1)!=0:
    ...
    for x in range(1, len(typeOne), 1):
        type_big_tree = my_big_tree.xpath('//div[@class="menu_box"]')[x]
        # 一级分类
        class_one_x = type_big_tree.xpath('//div[@class="menu_main job_hopping"]/h2')[x]
        ...
        for b in class_twoAndthree_x:
            ...
            for c in class_three_list:
                ...
                print("url:" + three_url)
                this_big_tee = header_browser(True, urlPool, three_url, User_Agent, Accept, Accept_Charset, Accept_Encoding, Host)
                # job列表
                ...
                if len(total_page1) >= 1:
                    ...
                    if total_page>=1:
                        for page in xrange(1, (int(total_page)+1), 1):
                            ...
                            print this_url
                            para = {'filterOption':3}
                            this_page_job_list = header_browser1(False, urlPool, para, this_url, User_Agent, Accept, Accept_Charset, Accept_Encoding, Host)
                            ...
                            for link in job_link_list:
                                ...
                                print link
                                # 详细职位介绍
                                detail_job = header_browser(False,urlPool,link, User_Agent, Accept, Accept_Charset, Accept_Encoding, Host)
